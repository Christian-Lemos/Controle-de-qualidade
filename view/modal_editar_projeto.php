<?php
	...
	$projDAO = new ProjetoDAO($con);
	$usuDAO = new UsuarioDAO($con);
	$projeto = $projDAO->encontrarProjeto($_POST['id']);
	$usuarios = $usuDAO->getUsuarios();
	$desenvolvedores = json_decode($projeto->getDesenvolvedores());
	$gerenteOpcoes =  '';
	$devsOpcoes = '';
	while($linha = $usuarios->fetch(PDO::FETCH_OBJ))
	{
		$devsOpcoes .= '<option value = "'.$linha->id.'" ';
		$gerenteOpcoes .= '<option value = "'.$linha->id.'" ';
		
		if($linha->id == $projeto->getGerente())
		{
		...
		{
			if($desenvolvedores[$i]->idusuario == $linha->id)
			{
				$devsOpcoes .= 'selected';
				break;
			}
		...
		$gerenteOpcoes .= '>'.$linha->nome.'</option>';
		$devsOpcoes .= '>'.$linha->nome.'</option>';		
	}
...
$(document).ready(function()
{
	...
	$("#edit_projeto_form_nome").on("submit", function()
	{
		...
		$.ajax
		({
			url : 'controller/atualizadorProjetos.php',
			method : 'POST',
			data : dadonovo,
