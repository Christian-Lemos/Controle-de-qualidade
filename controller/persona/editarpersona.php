<?php
    ...
    if(file_exists($_FILES['perfil']['tmp_name']) || is_uploaded_file($_FILES['perfil']['tmp_name']))
    {
        define('MB', 1048576);
        $target_dir = '../uploads/'; 
        ...
        {
            $dao = new PersonaDAO();
            $dao->atualizarPersona($_POST['id'], $_POST['nome'], $_POST['descricao'], 'uploads/'.$_FILES['perfil']['name'].$imageFileType);
            echo "sucesso";
        }
    ...
    {
        $dao = new PersonaDAO();
        $dao->atualizarPersona($_POST['id'], $_POST['nome'], $_POST['descricao'], NULL);
        echo "sucesso";
    }
